...
public class BuyerDAO {
    ...
    public List<Producer> serchPder(String search,String type){
        ...
        try {
            ...
            if(type.equals("option1")){
                sql="select * from FACTORY where FACTORYNAME like '%"+search+"%'";
            }else if(type.equals("option2")){
                sql="SELECT * FROM FACTORY WHERE FACTORYID='"+search+"'";
            }else {
    ...
    public synchronized boolean update(Staff staff){
        ...
        try{
            ...
            String sql="update EMPLOYEE set NAME='"+staff.getName()+"',PASSWORD='"
                    +staff.getPsd()+"',PHONE='"+staff.getPhone()+"'where EMPLOYEEID='"+staff.getID()+"'";
    ...
    public Producer CreateProByID(String id){
        ...
        try {
            connection=DBconn.getConnInstance(Constant.buyer);
            String sql="SELECT * FROM FACTORY WHERE FACTORYID=?";
            preparedStatement=connection.prepareStatement(sql);
            preparedStatement.setString(1,id);
            resultSet=preparedStatement.executeQuery();
            while (resultSet.next()){
    ...
    public boolean updateFirm(Producer producer){
        ...
        try{
            connection=DBconn.getConnInstance(Constant.buyer);
            statement=connection.createStatement();
            String sql="update FACTORY set FACTORYNAME='"+producer.getProducerName()+"',ADDRESS='"
                    +producer.getProducerAddress()+"',PHONE='"+producer.getProducerPhone()+
                    "'where FACTORYID='"+producer.getProducerID()+"'";
            int row=statement.executeUpdate(sql);
            if(row>0){
                flag=true;
            }
    ...
    public List<Drug> getMedicineByID(String id){
        ...
        try{
            connection=DBconn.getConnInstance(Constant.Administrator);
            String sql="select MEDICINEID,MEDICINENAME FROM MEDICINE where PRODUCERID=?";
            preparedStatement=connection.prepareStatement(sql);
            preparedStatement.setString(1,id);
            resultSet=preparedStatement.executeQuery();
            while (resultSet.next()){
