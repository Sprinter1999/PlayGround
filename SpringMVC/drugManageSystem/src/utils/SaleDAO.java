...
public class SaleDAO {
    ...
    public List<Customer> serchCutr(String search,String type){
        ...
        try {
            ...
            if(type.equals("option1")){
                sql="select * from CLIENT where NAME like '%"+search+"%'";
            }else if(type.equals("option2")){
                sql="SELECT * FROM CLIENT WHERE CLIENTID='"+search+"'";
            }else {
    ...
    public boolean updateCustom(Customer customer){
        ...
        try{
            ...
            String sql="update CLIENT set NAME='"+customer.getCustomerName()+"',PHONE='"+customer.getCustomerPhone()+
                    "'where CLIENTID='"+customer.getCustomerID()+"'";
    ...
    public List<MedicineTable> getAllMedicine(){
        ...
        try{
            ...
            String sql="select MEDICINEID,MEDICINENAME,库存总量,种类名称 FROM view_admin_药品总表";
            preparedStatement=connection.prepareStatement(sql);
            resultSet=preparedStatement.executeQuery();
            while (resultSet.next()){
