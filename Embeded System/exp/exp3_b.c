...
static int shell_show_history(struct uart_shell *shell) {
	...
	ush_printf(shell, "\r\n"); //clean line, for windows
	//ush_printf(shell, "\033[2K\r");/*this works fine */
	ush_printf(shell, " history: %s%s", SHELL_PROMPT, shell->line);
	return 0;
}
...
void print_shell_line(struct uart_shell *shell)
{
	ush_printf(shell,CR);
}
// ??????,????????????
...
//
	...
	return shell->line;
				...
				/*
				switch (ch) {
					//UP key:find the latest upper one
					case 0x41:
						if (shell->current_history > 0) {
							shell->current_history--;
							memcpy(shell->line,
					...
					//Down key:find next newer one
						...
						break;
					case 0x44:
					//left key:tuige and delete
						if (shell->line_curpos > 0) {
							shell->line_curpos--;
							print_shell_line(shell);
						}
						break;
					case 0x43:
					//right key print current char
				...
				break;
			case '\r':
			//huiche push all history
				shell_push_history(shell);
				go = 0;
				break;
			case SHELL_ESCAPSE_CHAR:
			//en xia jian mei yong
				return NULL;
				break;
			case 0x7F:
				shell->line_curpos = shell->line_curpos > 0 ? shell->line_curpos - 1 : 0;
				shell->line_position = shell->line_curpos;
				shell->line[shell->line_curpos] = '\0';	
				shell_show_history(shell);
				ch = shell->_getchar();
				break;
			case '\t':
